...
namespace mxProject.Devs.DataGeneration.ModelMappings.Fields
{
    ...
    public class SequenceTimeSpanFieldAttribute : SequenceFieldAttribute
    {
        ...
        public SequenceTimeSpanFieldAttribute(string initialValue, string maxValue, string increment, double nullProbability = 0, string? fieldName = null)
        ...
        {
            TimeSpan initial = context.StringConverter.ConvertTo<TimeSpan>(InitialValue);
            TimeSpan maxValue = context.StringConverter.ConvertTo<TimeSpan>(MaximumValue);
            TimeSpan increment = context.StringConverter.ConvertTo<TimeSpan>(Increment);

            return context.FieldFactory.SequenceTimeSpan(FieldName ?? member.Name, initial, maxValue, increment, NullProbability);
        }
