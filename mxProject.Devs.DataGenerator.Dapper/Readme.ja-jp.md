...
* このライブラリは `Dapper` を用いて `mxProject.Devs.DataGenerator` にO/Rマッピングの機能を追加します。

## 使用方法

### 単純なマッピング

* 生成されたデータを一つの型にマッピングします。

* DataGenerator クラスに対する拡張メソッド Map&lt;T&gt; を呼び出します。 
...
上記のメソッドを実行すると、次のような結果が出力されるでしょう。
...
* 生成されたデータを複数の型にマッピングします。

* DataGenerator クラスに対する拡張メソッド MapByFieldIndex&lt;T1, T2, ..., TResult&gt; または MapByFieldName&lt;T1, T2, ..., TResult&gt; を呼び出します。 
...
* mxProject.Devs.DataGenerator (>= 0.9.0)
* Dapper (>= 2.0.213)
